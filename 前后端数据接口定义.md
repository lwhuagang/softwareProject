...
| 软件反馈             | /user/feedback      | userEmail、feedbackMsg                   | 返回是否成功                                                 | POST     |
| 自选列表             | /user/watchList     | email                                    | 返回用户关注的所有基金信息的JSON数据（基金在列表界面的显示信息） | GET      |
| 持有列表             | /user/havingList    | email                                    | 返回用户持有的所有基金信息的JSON数据                         | GET      |
| 每日基金数据计算     | /user/calculate     | 无                                       | 无（计算所有用户的持有和收益的相关信息）                     | GET      |
| 添加搜索记录         | /user/addSearch     | userEmail、searchString                  | 返回是否插入成功                                             | POST     |
| 查询用户所有搜索记录 | /user/getAllSearch  | userEmail                                | 返回该用户的所有的搜索历史                                   | GET      |
...
| 计算今日交易 | /fundOperation/update | 无                         | 无（计算今日的交易，更新数据库中的相关结果）             |          |
